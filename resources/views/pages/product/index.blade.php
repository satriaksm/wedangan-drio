            ...
            <div class="mb-4 overflow-x-auto">
                <div class="sm:justify-normal flex gap-1 sm:gap-3">
                    <button
                        ...
                        :class="activeCategory === 'all' ? 'bg-primary' : 'bg-tertiary'"
                        class="text-white rounded-full py-2 px-4 sm:p-3 hover:bg-secondary transition-colors">
                        <h6 class="text-sm sm:text-base">Semua</h6>
                    </button>
                    @foreach ($categories as $category)
                        <button
                            @click="setActiveCategory('{{ $category->id }}')"
                            :class="activeCategory === '{{ $category->id }}' ? 'bg-primary' : 'bg-tertiary'"
                            class="text-white rounded-full py-2 px-4 sm:p-3 hover:bg-secondary transition-colors">
                    ...
                    <div
                        ...
                        <div class="flex gap-2 items-center w-full">
                            ...
                            <div class="flex flex-col items-center gap-2">
                                <div class="basis-1/2 w-full rounded-xl max-h-12">
                                    <a href="{{route('product.edit',$item->id)}}" class="">
                                        <button class="text-white font-bold w-full p-3 bg-yellow-200 rounded-xl">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" fill="currentColor" class="w-5 h-5 md:w-6 md:h-6 mx-auto">
                                ...
                                <div class="basis-1/2 bg-red-500 w-full rounded-xl max-h-12">
                                    <form action="{{ route('product.destroy', $item->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button class="text-white font-bold w-full p-3" type="submit">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" fill="currentColor" class="w-5 h-5 md:w-6 md:h-6 mx-auto">
